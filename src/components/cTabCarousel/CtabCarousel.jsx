...
const CtabCarousel = ({ marker }) => {
  ...
  const [navigateIsImg, setNavigateIsImg] = useState(false);
          ...
          if (item.projects?.length) {
            return (
              <div
                ...
                onMouseMove={e => {
                  setNavigateIsImg(true);
                  e.stopPropagation();
                }}
                onMouseLeave={e => {
                  setNavigateIsImg(false);
                  e.stopPropagation();
                }}
                key={index}>
                <Swiper
                  // style={isSmallScreen ? { left: "-38px" } : ""}
                  ref={swiperRef}
                  className="mySwiper c-tab-hidden">
                  {item.projects.map(content => {
                    if (typeof content === "object" && content) {
                      return (
                        <SwiperSlide key={content.id}>
                          <div className="swiper_content swiper_projects">
                            <div className="siper_content_left">
                              ...
                              <div
                                className={`swiper_ccontent_left ${
                                  navigateIsImg ? "active_img" : ""
                                }`}>
                                <h3
