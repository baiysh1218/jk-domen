...
const PageContextProvider = ({ children }) => {
  ...
  async function getTeamInfoAll() {
    try {
      ...
      const result = await axios(
        `${API}/${state.language}/team/all/${window.location.search}`
      );
  ...
  const vacancies = async () => {
    try {
      // console.log(window.location.search);
      const result = await axios(
        `${API}/${state.language}/vacancies/all?${window.location.search}`
      );
      console.log(result);
      const totalPage = Math.ceil(result.data.count / 5);
      dispatch({ type: "VACANCIES", payload: result.data });
      // dispatch({ type: "PAGE_TOTAL_COUNT", payload: totalPage });
    } catch (error) {
      console.error(error);
    }
  };
